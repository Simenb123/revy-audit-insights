...
deno.test("normal query returns articles", async () => {
  ...
  const depsMod = await import("../../deps.ts");
  const createStub = stub(depsMod, "createClient", createClient);
  const serveStub = stub(depsMod, "serve", () => {});
...
deno.test("query with punctuation is sanitized", async () => {
  ...
  const depsMod = await import("../../deps.ts");
  const createStub = stub(depsMod, "createClient", () => supabaseStub);
  const serveStub = stub(depsMod, "serve", () => {});
