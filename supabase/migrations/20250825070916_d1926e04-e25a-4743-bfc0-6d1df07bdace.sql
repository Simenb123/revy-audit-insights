-- Migrasjon for Aksjonærregister - Fix syntax error
...
-- Opprett unique constraint for share_companies
CREATE UNIQUE INDEX IF NOT EXISTS unique_share_companies_orgnr_year_user 
...
CREATE TABLE IF NOT EXISTS public.share_entities (
  ...
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  created_at timestamptz DEFAULT now(),
  -- Legger til computed kolonne for unikhet
  entity_key text GENERATED ALWAYS AS (COALESCE(orgnr, name || ':' || COALESCE(birth_year::text, ''))) STORED
);

-- Opprett unique constraint for share_entities
CREATE UNIQUE INDEX IF NOT EXISTS unique_share_entities_key_user 
ON public.share_entities(entity_key, user_id);
...
-- Opprett unique constraint for share_holdings
CREATE UNIQUE INDEX IF NOT EXISTS unique_share_holdings_company_holder_class_year_user 
