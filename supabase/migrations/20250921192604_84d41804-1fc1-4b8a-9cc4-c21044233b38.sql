-- Optimize process_shareholders_batch function for streaming processing
-- Drop and recreate with better memory management
...
BEGIN
  -- Log function start with reduced limit for better memory management
  RAISE NOTICE 'Processing shareholders batch (streaming optimized): job_id=%, user_id=%, offset=%, limit=%', 
    p_job_id, p_user_id, p_offset, p_limit;

  -- Reduce batch size even further to prevent memory issues
  v_batch_size := LEAST(p_limit, 500); -- Max 500 rows per batch for streaming
  ...
  IF v_processed_count > 0 THEN
    ...
    BEGIN
      -- 1) COMPANIES: Insert/update companies from staging data with DISTINCT subquery
      ...
      -- 2) ENTITIES: Handle companies (9-digit orgnr) with DISTINCT subquery
      ...
      -- 3) ENTITIES: Handle persons (4-digit birth year) with DISTINCT subquery
