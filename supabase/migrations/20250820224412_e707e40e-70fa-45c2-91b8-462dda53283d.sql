...
FOR ALL
USING (
  get_user_role(auth.uid()) = ANY(ARRAY['admin', 'partner', 'manager']) 
...
BEGIN
  ...
  SELECT p.audit_firm_id INTO target_firm_id
  FROM public.profiles p
  ...
  IF requester_role NOT IN ('admin', 'partner') THEN
  ...
  IF p_new_role = 'admin' AND requester_role != 'admin' THEN
  ...
  INSERT INTO public.admin_audit_logs (
    ...
    format('Role changed from %s to %s', (SELECT user_role FROM public.profiles WHERE id = p_user_id), p_new_role),
