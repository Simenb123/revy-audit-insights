...
-- 7. Create security monitoring function (instead of view)
...
AS $function$
  ...
  SELECT 
    'admin_actions'::text as metric_type,
    COUNT(*) as count,
    DATE_TRUNC('day', created_at)::date as date
  FROM public.admin_audit_logs 
  WHERE created_at >= CURRENT_DATE - INTERVAL '30 days'
    AND get_user_role(auth.uid()) = ANY (ARRAY['admin'::user_role_type, 'partner'::user_role_type])
  GROUP BY DATE_TRUNC('day', created_at)
  UNION ALL
  SELECT 
    'failed_logins'::text as metric_type,
    COUNT(*) as count,
    DATE_TRUNC('day', created_at)::date as date
  FROM public.admin_audit_logs 
