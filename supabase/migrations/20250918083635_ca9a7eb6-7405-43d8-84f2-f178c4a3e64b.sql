-- Create process_shareholders_batch function to handle batch processing of shareholder data
-- This moves heavy data processing from Edge Function to PostgreSQL for better performance
...
BEGIN
  ...
  IF v_processed_count > 0 THEN
    ...
    BEGIN
      ...
      -- 3) ENTITIES: Handle persons (4-digit birth year)  
      INSERT INTO share_entities (entity_key, name, orgnr, birth_year, country_code, entity_type, user_id)
      SELECT DISTINCT
        LOWER(TRIM(navn_aksjonaer)) || '|' || COALESCE(NULLIF(TRIM(fodselsaar_orgnr), ''), '?') AS entity_key,
        NULLIF(TRIM(navn_aksjonaer), '') AS name,
...
GRANT EXECUTE ON FUNCTION process_shareholders_batch(BIGINT, UUID, INTEGER, INTEGER) TO authenticated;

-- Add comment for function documentation
COMMENT ON FUNCTION process_shareholders_batch(BIGINT, UUID, INTEGER, INTEGER) IS 
'Processes shareholder data in batches from staging to production tables. Uses user_id for security.';