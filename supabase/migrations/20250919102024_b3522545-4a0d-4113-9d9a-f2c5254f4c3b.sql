-- Fix process_shareholders_batch function to use correct column names
...
BEGIN
  ...
  IF v_processed_count > 0 THEN
    ...
    BEGIN
      ...
      -- 5) Update job progress (removed updated_at references)
      UPDATE import_jobs 
      SET 
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Log error and update job status (changed error_message to error, removed updated_at)
    ...
    END;
  ELSE
    -- No rows to process, mark as done if we're at or beyond the end (removed updated_at)
    v_is_done := TRUE;
