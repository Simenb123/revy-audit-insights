-- Create a function to trigger AI processing when text extraction completes
...
DROP TRIGGER IF EXISTS trigger_document_ai_processing_on_completion ON public.client_documents_files;

-- Create the trigger on the client_documents_files table
CREATE TRIGGER trigger_document_ai_processing_on_completion
    AFTER UPDATE ON public.client_documents_files
    FOR EACH ROW
    EXECUTE FUNCTION public.trigger_document_ai_processing();

-- Create a function to manually trigger AI processing for existing completed documents
