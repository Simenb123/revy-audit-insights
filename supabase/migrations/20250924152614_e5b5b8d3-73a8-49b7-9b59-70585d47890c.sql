-- Fix entity_key GENERATED ALWAYS conflict in process_shareholders_batch function
-- Remove entity_key from INSERT statements since it's auto-generated
...
BEGIN
  -- Log function start
  RAISE NOTICE 'Processing shareholders batch (entity_key fix): job_id=%, user_id=%, offset=%, limit=%', 
  ...
  IF v_processed_count > 0 THEN
    ...
    BEGIN
      ...
      -- 2) ENTITIES: Handle companies (9-digit orgnr) - LET DATABASE GENERATE entity_key
      ...
      -- 3) ENTITIES: Handle persons with SAFE birth_year conversion - LET DATABASE GENERATE entity_key
      WITH staged_person_entities AS (
        SELECT DISTINCT ON (name, birth_year, user_id)
          NULLIF(TRIM(navn_aksjonaer), '') AS name,
          NULL AS orgnr,
        ...
        WHERE user_id = p_user_id
          ...
          AND s.id > p_offset 
          AND s.id <= (p_offset + v_batch_size)
        ORDER BY name, birth_year, user_id, id
      )
      INSERT INTO share_entities (name, orgnr, birth_year, country_code, entity_type, user_id)
      SELECT name, orgnr, birth_year, country_code, entity_type, user_id FROM staged_person_entities
      ON CONFLICT (name, birth_year, user_id) DO UPDATE SET
        country_code = EXCLUDED.country_code;

      GET DIAGNOSTICS v_processed_count = ROW_COUNT;
      RAISE NOTICE 'Inserted/updated % person entities', v_processed_count;

      -- 4) HOLDINGS: Insert share holdings with proper entity joins using entity_key
