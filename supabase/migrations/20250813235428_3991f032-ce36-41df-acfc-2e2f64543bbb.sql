...
create policy if not exists "owner_can_manage_settings"
  on public.report_builder_settings
  for all
  using (user_id = auth.uid())
  with check (user_id = auth.uid());

-- Helpful read-only policy (redundant due to ALL, but explicit)
create policy if not exists "owner_can_read_settings"
  on public.report_builder_settings
  for select
  using (user_id = auth.uid());
