...
-- Fix process_shareholders_batch to not use entity_type casting
-- The entity_type column is TEXT, not an enum type
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            -- Determine entity type (TEXT, not enum)
            v_entity_type := CASE 
                WHEN length(v_staging_row.fodselsaar_orgnr) = 9 THEN 'company'
                WHEN length(v_staging_row.fodselsaar_orgnr) = 4 THEN 'person'
                ELSE 'person'
            END;
        ...
        EXCEPTION WHEN OTHERS THEN
            ...
            -- Log the actual error (less verbose now)
