-- Fix CTE scope issue in optimized_analysis function
...
BEGIN
  ...
  -- Get the latest trial balance version for this client
  ...
  -- Get trial balance data separately
  SELECT 
    COUNT(*),
  ...
  -- Set ledger vs trial balance check values
  v_ledger_balance := v_total_net;
  v_trial_balance_net := v_tb_total_net;

  -- Build aggregated data from CTEs
  WITH monthly_agg AS (
    SELECT 
