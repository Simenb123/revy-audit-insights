-- Core Security Fixes Migration
-- Phase 1: Critical Security Protections

-- 1. Secure Legal Documents Access (if tables exist)
DO $$
BEGIN
    -- Check if legal_documents table exists and add RLS policies
    ...
    -- Check if legal_citations table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'legal_citations') THEN
    ...
    -- Check if knowledge_article_tags table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'knowledge_article_tags') THEN
...
BEGIN
    ...
    -- Ensure user_role has proper default
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- 3. Secure Database Functions - Add SET search_path to critical functions
-- Update get_user_role function (already exists from supabase info)
...
-- 6. Add rate limiting table for sensitive operations
