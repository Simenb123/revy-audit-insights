-- Create comprehensive population analysis function
...
BEGIN
  ...
  WITH filtered_balances AS (
    ...
    JOIN public.trial_balance_mappings tbm ON tb.account_number = tbm.account_number 
      AND tb.client_id = tbm.client_id
    JOIN public.standard_accounts sa ON tbm.standard_account_id = sa.id
    WHERE tb.client_id = p_client_id
      AND tb.period_year = p_fiscal_year
      AND tb.version_id = v_version_id
      AND sa.standard_number = ANY(p_selected_standard_numbers)
      AND NOT (tb.account_number = ANY(p_excluded_account_numbers))
  ),
  stats AS (
    SELECT 
  ...
  SELECT jsonb_build_object(
    ...
    'accountsWithBalance', s.accounts_with_balance,
  ...
  WITH transaction_pairs AS (
    SELECT 
      ...
      glt2.account_name as counter_account_name,
    ...
    JOIN public.general_ledger_transactions glt2 ON glt1.voucher_number = glt2.voucher_number 
      ...
      AND glt1.account_number != glt2.account_number
    JOIN public.trial_balance_mappings tbm ON glt1.account_number = tbm.account_number 
      AND glt1.client_id = tbm.client_id
    JOIN public.standard_accounts sa ON tbm.standard_account_id = sa.id
    WHERE glt1.client_id = p_client_id
      AND EXTRACT(YEAR FROM glt1.transaction_date) = p_fiscal_year
      AND glt1.version_id = v_version_id
      AND sa.standard_number = ANY(p_selected_standard_numbers)
      AND NOT (glt1.account_number = ANY(p_excluded_account_numbers))
    GROUP BY glt1.account_number, glt2.account_number, glt2.account_name
  )
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  WITH monthly_data AS (
    ...
    JOIN public.trial_balance_mappings tbm ON glt.account_number = tbm.account_number 
      AND glt.client_id = tbm.client_id
    JOIN public.standard_accounts sa ON tbm.standard_account_id = sa.id
  ...
  WITH account_details AS (
    ...
    ) tc ON tb.account_number = tc.account_number
    WHERE tb.client_id = p_client_id
      AND tb.period_year = p_fiscal_year
      AND tb.version_id = v_version_id
      AND sa.standard_number = ANY(p_selected_standard_numbers)
...
CREATE INDEX IF NOT EXISTS idx_trial_balance_mappings_lookup 
ON public.trial_balance_mappings (client_id, account_number, standard_account_id);
