-- Optimize the calculate_population_analysis function to handle versions correctly and improve performance
CREATE OR REPLACE FUNCTION public.calculate_population_analysis(
    ...
    p_version_string TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Return analysis with proper error handling
    RETURN jsonb_build_object(
...
EXCEPTION
    WHEN OTHERS THEN
        -- Return empty result on any error to prevent function crashes
        RETURN jsonb_build_object(
