-- Fix import_jobs update statement by removing non-existent updated_at column
...
BEGIN
  ...
  -- Update job status in import_jobs table (removed updated_at since column doesn't exist)
  UPDATE public.import_jobs
  SET 
    rows_loaded = rows_loaded + v_processed,
    status = CASE WHEN v_errors > 0 THEN 'error' ELSE status END
  WHERE id = p_job_id;
