-- Enhanced calculate_population_analysis function to include detailed analyses
...
BEGIN
    ...
    WITH counter_account_analysis AS (
        SELECT 
            CASE 
                WHEN glt.debit_account = ANY(v_account_numbers) THEN glt.credit_account
                WHEN glt.credit_account = ANY(v_account_numbers) THEN glt.debit_account
                ELSE NULL
            END as counter_account,
            SUM(glt.amount) as total_amount,
        ...
        WHERE glt.client_id = p_client_id
          ...
          AND (glt.debit_account = ANY(v_account_numbers) OR glt.credit_account = ANY(v_account_numbers))
          AND glt.amount > 0
        GROUP BY counter_account
        HAVING counter_account IS NOT NULL
    ...
    WITH monthly_data AS (
        ...
        WHERE glt.client_id = p_client_id
          AND EXTRACT(YEAR FROM glt.transaction_date) = p_fiscal_year
          AND (glt.debit_account = ANY(v_account_numbers) OR glt.credit_account = ANY(v_account_numbers))
        GROUP BY EXTRACT(YEAR FROM glt.transaction_date), EXTRACT(MONTH FROM glt.transaction_date)
        ORDER BY year, month
    )
    SELECT jsonb_agg(
        jsonb_build_object(
    ...
    -- Anomaly detection (basic patterns)
        ...
        SELECT 
            ...
            CASE 
                WHEN pb.closing_balance > 0 AND NOT EXISTS(
                    ...
                    WHERE glt.client_id = p_client_id 
                    AND glt.credit_account = pb.account_number 
                    AND EXTRACT(YEAR FROM glt.transaction_date) = p_fiscal_year
                ) THEN 'only_debits'
                WHEN pb.closing_balance < 0 AND NOT EXISTS(
                    SELECT 1 FROM public.general_ledger_transactions glt 
                    WHERE glt.client_id = p_client_id 
                    AND glt.debit_account = pb.account_number 
                    AND EXTRACT(YEAR FROM glt.transaction_date) = p_fiscal_year
                ) THEN 'only_credits'
