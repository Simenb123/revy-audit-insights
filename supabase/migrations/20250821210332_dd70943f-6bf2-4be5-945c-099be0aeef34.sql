-- Fix RLS policies with unique names to allow proper CRUD operations
...
DROP POLICY IF EXISTS "Authenticated users can manage legal provisions" ON public.legal_provisions;

CREATE POLICY "Legal provisions full access for authenticated users" 
...
CREATE POLICY "Legal documents full access for authenticated users" 
...
DROP POLICY IF EXISTS "Authenticated users can manage legal citations" ON public.legal_citations;

CREATE POLICY "Legal citations full access for authenticated users" 
...
DROP POLICY IF EXISTS "Authenticated users can manage knowledge article tags" ON public.knowledge_article_tags;

CREATE POLICY "Knowledge article tags full access for authenticated users" 
