-- Update calculate_population_analysis function to include trendAnalysis in response
...
BEGIN
    ...
    WITH trend_calculation AS (
        SELECT 
            ...
            CASE 
                ...
                WHEN STDDEV(total_amount::numeric) / NULLIF(AVG(total_amount::numeric), 0) < 0.1 THEN 'stable'
                WHEN (SELECT total_amount FROM monthly_data ORDER BY year DESC, month DESC LIMIT 1) > 
                     (SELECT AVG(total_amount) FROM monthly_data) THEN 'increasing'
                ELSE 'decreasing'
            END as trend,
            CASE 
                WHEN COUNT(*) >= 12 AND STDDEV(total_amount::numeric) / NULLIF(AVG(total_amount::numeric), 0) > 0.2 THEN 'detected'
                ELSE 'none'
            END as seasonality
        FROM monthly_data
    )
    ...
    -- Return comprehensive analysis with trend analysis included
    RETURN jsonb_build_object(
