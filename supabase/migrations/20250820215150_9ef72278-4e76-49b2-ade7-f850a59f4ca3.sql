-- SECURITY FIX: Remove Security Definer Views
-- Views owned by postgres superuser bypass RLS policies and can expose data inappropriately
-- We need to drop and recreate these views with proper ownership and security

-- Drop existing views that are causing security definer issues
...
-- Recreate views with proper security context (no SECURITY DEFINER)
...
-- Note: These views will now properly respect RLS policies on the underlying tables
-- instead of bypassing them through superuser ownership