...
export class KnowledgeSearchDiagnostics {
  ...
  static async runHealthCheck(): Promise<KnowledgeBaseHealth> {
    if (!isSupabaseConfigured || !supabase) {
      logger.error("Supabase is not configured. Health check cannot run.");
      throw new Error("Supabase not initialized");
    }
    logger.log('🏥 Running knowledge base health check...');
    ...
    try {
      ...
      try {
        ...
        if (error) {
          ...
          logger.log('✅ Search function working, returned', data.articles.length, 'results');
        }
      } catch (error: any) {
        logger.error('❌ Search function exception:', error);
      ...
      logger.log('📊 Health check completed in', Date.now() - startTime, 'ms:', health);
      return health;
    } catch (error) {
  ...
  static async runSyntheticQueries(): Promise<SearchDiagnostic[]> {
    if (!isSupabaseConfigured || !supabase) {
      logger.error("Supabase is not configured. Cannot run synthetic queries.");
      return [];
    }
    logger.log('🤖 Running synthetic query tests...');
  ...
  static async testSingleQuery(query: string): Promise<SearchDiagnostic> {
    if (!isSupabaseConfigured || !supabase) {
      logger.error("Supabase is not configured. Cannot test query.");
    ...
    logger.log(`🔍 Testing query: "${query}"`);
    const startTime = Date.now();
    const errors: string[] = [];
