...
const SortableHeader: React.FC<{ def: DataTableColumn<T>; pinned: boolean; sticky: boolean }> = ({ def, pinned, sticky }) => {
  ...
  return (
    <TableHead
      ...
      className={`${def.sortable !== false ? 'cursor-pointer hover:bg-muted/50 select-none' : ''} ${def.align === 'right' ? 'text-right' : def.align === 'center' ? 'text-center' : ''} ${def.className || ''} ${(sticky || pinned) ? 'sticky' : ''} ${sticky ? 'top-0 z-30 bg-background' : ''} ${pinned ? 'left-0 z-40 bg-background' : ''} relative`}
      style={style}
