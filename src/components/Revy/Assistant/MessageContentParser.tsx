...
interface MessageContentParserProps {
  content: string;
  isEmbedded?: boolean;
  /** Enable parsing of metadata such as variant info and document references */
  advanced?: boolean;
  /** Show document context banner when advanced parsing is enabled */
...
export const MessageContentParser = ({
  content,
  isEmbedded = false,
  advanced = false,
  showDocumentContext = true,
}: MessageContentParserProps) => {
  ...
  if (advanced) {
    hasDocumentContext =
      content.includes('DOKUMENTINNHOLD FUNNET') ||
  ...
  const processedElements = processContent(contentToProcess);

  return (
    <div className="space-y-2">
      {advanced && hasDocumentContext && showDocumentContext && (
      ...
      {advanced && knowledgeArticles.length > 0 && (
      ...
      {advanced && documentReferences.length > 0 && (
          ...
          <ul className="space-y-2">
            {documentReferences.map((doc) => (
              <li key={doc.id} className="border rounded-lg p-2 bg-gray-50">
                <div className="text-xs font-semibold text-gray-700">{doc.fileName}</div>
                {doc.snippet && (
                  <p className="text-xs text-gray-600 italic mt-1">"{doc.snippet}"</p>
                )}
                <a href="/documents" className="text-xs text-blue-600 hover:underline mt-1 inline-block">
                  Åpne dokument
                </a>
              </li>
            ))}
