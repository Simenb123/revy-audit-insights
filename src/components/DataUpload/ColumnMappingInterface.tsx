...
const defaultFields: StandardField[] = [
  {
    key: "transaction_date",
    label: "Dato",
    required: true,
    description: "Transaksjonsdato (dd.mm.åååå)",
  },
  {
    key: "voucher_number",
    label: "Bilagsnummer",
    required: false,
    description: "Journal-ID / Bilagsnummer",
  },
  {
    key: "voucher_type",
    label: "Bilagsart",
    required: false,
    description: "OB, SI, PI, CP, PP, PR, RENT, DEP, COGS, BC, INT",
  },
  {
    key: "description",
    label: "Beskrivelse",
    required: false,
    description: 'Bilagstekst (f.eks. "Sales invoice ...")',
  },
  {
    key: "customer_supplier_name",
    label: "Tekst",
    required: false,
    description: "Navn på kunde/leverandør",
  },
  {
    key: "account_number",
    label: "Kontonummer",
    required: true,
    description: "Norsk kontonummer",
  },
  {
    key: "account_name",
    label: "Kontonavn",
    required: false,
    description: "Kontonavn (brukes som fallback hvis kontonr mangler)",
  },
  {
    key: "vat_code",
    label: "MVA-kode",
    required: false,
    description: "03 = utgående 25%, 01 = inngående 25%",
  },
  {
    key: "vat_amount",
    label: "MVA-beløp",
  ...
  },
  {
    ...
    label: "Beløp",
    required: true,
    description: "+ Debet − Kredit (positiv for debet, negativ for kredit)",
  },
  ...
  // Legacy fields for backward compatibility
  {
    ...
    label: "Debet (legacy)",
    required: false,
    description: "Debetbeløp (eldre format)",
  },
  {
    key: "credit_amount",
    label: "Kredit (legacy)",
    required: false,
    description: "Kreditbeløp (eldre format)",
  },
  {
    key: "balance_amount",
    label: "Saldo (legacy)",
    required: false,
    description: "Saldo etter transaksjon (eldre format)",
  },
];
...
const ColumnMappingInterface = ({
  ...
  const standardFields = fields || defaultFields;
  ...
  useEffect(() => {
    ...
    fileColumns.forEach((column) => {
      ...
      if (lowerColumn.includes("dato") || lowerColumn.includes("date")) {
        ...
        lowerColumn.includes("bilag") ||
        lowerColumn.includes("voucher") ||
        lowerColumn.includes("journal")
      ) {
        autoMapping[column] = "voucher_number";
      } else if (
        lowerColumn.includes("bilagsart") ||
        lowerColumn.includes("type")
      ) {
        autoMapping[column] = "voucher_type";
      } else if (
        lowerColumn.includes("beskriv") ||
        lowerColumn.includes("description")
      ) {
        autoMapping[column] = "description";
      } else if (
        lowerColumn.includes("tekst") ||
        lowerColumn.includes("kunde") ||
        lowerColumn.includes("leverandør")
      ) {
      ...
      // Legacy mappings for backward compatibility
        ...
        lowerColumn.includes("kredit") ||
        lowerColumn.includes("credit")
      ) {
        autoMapping[column] = "credit_amount";
      } else if (
        lowerColumn.includes("saldo") ||
        lowerColumn.includes("balance")
      ) {
        autoMapping[column] = "balance_amount";
      } else if (
        lowerColumn.includes("periode") ||
        lowerColumn.includes("period")
      ) {
        autoMapping[column] = "period_end_date";
      } else if (
        lowerColumn.includes("inng") ||
        lowerColumn.includes("opening")
      ) {
        autoMapping[column] = "opening_balance";
      } else if (
        lowerColumn.includes("utg") ||
        lowerColumn.includes("closing")
      ) {
        autoMapping[column] = "closing_balance";
      } else if (
        lowerColumn.includes("debet") &&
        lowerColumn.includes("omset")
      ) {
        autoMapping[column] = "debit_turnover";
      } else if (
        lowerColumn.includes("kredit") &&
        lowerColumn.includes("omset")
      ) {
        autoMapping[column] = "credit_turnover";
      }
    });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Koble kolonnene fra filen din til standardfeltene. Påkrevde felter:{" "}
            ...
            <div>
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {standardFields.map((field) => {
                  ...
                  const mappedColumn = Object.keys(mapping).find(
                  ...
                  {sampleData.slice(0, 5).map((row, index) => (
                    <TableRow key={index}>
                      {standardFields
                        ...
                        .map((field) => {
                          const fileColumn = Object.keys(mapping).find(
                            (col) => mapping[col] === field.key,
                          );
                          return (
                            <TableCell
