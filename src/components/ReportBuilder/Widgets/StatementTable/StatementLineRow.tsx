...
const countVisible = React.useCallback((node: any): number => {
  ...
  return (
    <>
      {/* Children before self when subtotal line is expanded */}
      {hasChildren && isOpen && isTotal && (() => {
        let next = rowIndex; // children start at this node's first visible row
      ...
      {/* Self row */}
      {(() => {
        // Base row index for this line's own row
        let baseRowIndex = rowIndex;
        if (hasChildren && isOpen && isTotal) {
          // place subtotal row after all visible children
          const visibleChildren = line.children.reduce((acc: number, c: any) => acc + countVisible(c), 0);
          baseRowIndex = rowIndex + visibleChildren;
        }
...
onClick={(e) => { 
            ...
            {/* Inline accounts under this row (use baseRowIndex as anchor) */}
      ...
      {/* Children after self when normal (non-total) node is expanded */}
      {hasChildren && isOpen && !isTotal && (() => {
        const extraAccounts = accountsOpen ? accounts.length : 0;
        let next = rowIndex + 1 + extraAccounts;
        return line.children.map((child: any, idx: number, arr: any[]) => {
          const start = next;
          next += countVisible(child);
          return (
