...
export const useTrialBalanceWithMappings = (clientId: string, fiscalYear?: number, selectedVersion?: string) => {
  return useQuery({
    queryKey: ['trial-balance-with-mappings', clientId, fiscalYear, selectedVersion],
      queryFn: async () => fetchTrialBalanceWithMappings(clientId, fiscalYear, selectedVersion),
    enabled: !!clientId,
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
...
export async function fetchTrialBalanceWithMappings(clientId: string, fiscalYear?: number, selectedVersion?: string) {
  ...
  if (!selectedVersion || !fiscalYear) {
    ...
    if (versionData && versionData.length > 0) {
      ...
      const versionMap = new Map<any, any>();
      (versionData as any[]).forEach((item: any) => {
      ...
      let bestScore = 0;
      for (const [_key, info] of versionMap) {
  ...
  if (!trialBalanceData || (trialBalanceData as any[]).length === 0) {
  ...
  try {
    const { data: prevLatest } = await supabase
      ...
      .order('created_at', { ascending: false })
      .limit(1);
    if (prevLatest && (prevLatest as any[]).length > 0) {
  ...
  (prevData as any[])?.forEach((row: any) => {
  ...
  // Transform the data
  const trialBalanceEntries: TrialBalanceEntryWithMapping[] = (trialBalanceData as any[]).map((tb: any) => {
